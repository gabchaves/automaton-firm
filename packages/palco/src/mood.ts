 ...
 * from anything but real snapshot fields (status, bookMc).
 *
 * `STAKE_MC` mirrors `src/motor/cohort.ts`'s `TRADER_START_MC` ($20.00 per
 * hired trader) — same mirroring rule as `types.ts` (packages/palco is a
 * standalone workspace, no cross-package import into `src/motor/`).
 *
 ...
 */
export const STAKE_MC = 2_000_000; // $20.00, mirrors TRADER_START_MC
...
export function moodEmoji(status: string, bookMc: number): string {
  if (status === "dead") return MOOD_DEAD;
  if (status === "fired") return MOOD_FIRED;

  const ratio = bookMc / STAKE_MC;
